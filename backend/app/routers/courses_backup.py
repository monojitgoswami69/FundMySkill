"""
OPTIMIZED Courses router - FAST Firebase queries
Performance improvements:
1. Use Firestore where() clauses instead of fetching all + filtering
2. Batch operations for related data
3. Limit fields fetched (select())
...
async def get_courses(
    ...
    """
    OPTIMIZED: Get paginated list of courses with Firestore WHERE clauses.
    Returns courses formatted for the CourseCataloguePage.

    Performance improvements:
    - Uses Firestore where() instead of fetching all
    - Limits to 100 docs max to prevent huge fetches
    - Only published courses
    """
    db = get_firestore()

    # Start with base query - ONLY published courses
    query = db.collection("courses").where("status", "==", "published")

    # Apply Firestore filters (much faster than Python filtering)
    if category:
    ...
    # Limit fetch to prevent huge queries
    query = query.limit(100)

    # Execute query
    docs = list(query.stream())

    # Filter courses (only search filter needs Python, rest done by Firestore)
    courses = []
    for doc in docs:
        data = doc.to_dict()
        if not data:
...
async def get_categories() -> dict:
    ...
    docs = list(db.collection("courses").where("status", "==", "published").stream())
...
async def get_course(course_id: str) -> dict:
    """
    OPTIMIZED: Get full course details including modules and lectures.

    Performance improvements:
    - Uses collection group query to fetch all lectures in one go
    - Batch fetch modules
    - Reduces round trips from N+1 to 2 queries
    """
    db = get_firestore()
    ...
    # OPTIMIZED: Fetch all modules in one query
    module_docs = list(
    ...
    # OPTIMIZED: Fetch ALL lectures for this course in ONE query using collection group
    # This is MUCH faster than N queries (one per module)
